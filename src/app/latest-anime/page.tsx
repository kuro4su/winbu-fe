    ...
    return (
        <>
            ...
            <div className="container mx-auto px-4 py-8 space-y-8">
                ...
                {/* Content Grid */}
                <ContentGrid title="📺 ANIME DONGHUA" items={result.data.latest_anime} />
                    ...
                    <div className="brutal-border brutal-shadow-lg bg-background p-6">
                        <div className="flex items-center justify-center gap-2 flex-wrap">
                            ...
                            {result.pagination.prev_page ? (
                                <Link
                                    href={`/latest-anime?page=${result.pagination.prev_page}`}
                                        ...
                                        return (
                                            <div key={pageNum} className="flex items-center gap-2">
                                                ...
                                                {pageNum === result.pagination.current_page ? (
                                                    ...
                                                    <Link
                                                        href={`/latest-anime?page=${pageNum}`}
                                                        className="brutal-border brutal-shadow-sm bg-card px-4 py-2 font-black uppercase hover:brutal-shadow transition-all min-w-[3rem] text-center"
                                                    >
                                                        {pageNum}
                                                    </Link>
                                                )}
                            ...
                            {result.pagination.next_page ? (
                                <Link
                                    href={`/latest-anime?page=${result.pagination.next_page}`}
                                    className="brutal-border brutal-shadow-sm bg-card px-4 py-2 font-black uppercase hover:brutal-shadow transition-all"
                                >
                                    NEXT →
                                </Link>
